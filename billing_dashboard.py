...
class MainWindow(QMainWindow):
    ...
    def handle_analysis(self):
        ...
        def run_analysis():
            ...
            try:
                ...
                # QMessageBox.information(dialog, "Success", f"Successfully mapped {selected_sheet_col} to {selected_db_col} in {selected_table}")
            except Exception as e:
    ...
    def save_mapping(self, table_name, db_col, sheet_col, df, threshold=60):
        ...
        try:
            ...
            QMessageBox.warning(self, "Warning", "Incorrect Mapping for table.")
    ...
    def analyze_df(self, df, holiday_map):
        ...
        self.show_dashboard(output_list)
        return output_list

    def show_dashboard(self, output_list):
        ...
        def kpi_card(title, value, color="primary"):
            return dbc.Card(dbc.CardBody([html.H6(title, className="card-subtitle mb-2 text-muted kpi-title"),
                html.H4(f"{value}", className=f"card-title text-{color} font-weight-bold kpi-title"), ],
                className="text-center", ), className="mb-3 shadow-sm kpi-card", )

        dash_app.layout = dbc.Container(# # Inject custom CSS
            # html.Div([html.Style(custom_style)]),
            fluid=True, children=[  # Header
                dbc.Row([dbc.Col(
                    [html.H1("Employee Leave & Attendance Dashboard", className="text-center my-4 text-primary", ),
                        html.P("Comprehensive analysis of employee leave patterns and attendance trends",
                            className="text-center text-muted mb-4", ), ])]), # Filters and Controls
                dbc.Card([dbc.CardBody([dbc.Row([dbc.Col([dbc.Label("Employee", html_for="employee-dropdown"),
                    dcc.Dropdown(id="employee-dropdown", options=[{"label": n, "value": n} for n in employees],
                        value=employees[0], clearable=False, className="mb-2", ), ], width=3, ), dbc.Col(
                    [dbc.Label("Year", html_for="year-dropdown"),
                        dcc.Dropdown(id="year-dropdown", options=[{"label": int(y), "value": int(y)} for y in years],
                            value=years[-1], clearable=False, className="mb-2", ), ], width=2, ), dbc.Col(
                    [dbc.Label("Calculation Base", html_for="calculation-base-dropdown", ),
                        dcc.Dropdown(id="calculation-base-dropdown",
                            options=[{"label": "Total Working Days", "value": "Total Working Days", },
                                {"label": "Total Billable Days", "value": "Total Billable Days", }, ],
                            value="Total Working Days", clearable=False, className="mb-2", ), ], width=2, ), dbc.Col(
                    [dbc.Label("Attendance Threshold (%)", html_for="threshold-slider", ),
                        dcc.Slider(id="threshold-slider", min=50, max=100, step=1, value=90,
                            marks={x: str(x) for x in range(50, 101, 10)},
                            tooltip={"placement": "bottom", "always_visible": True, }, ), ], width=3, ), dbc.Col([
                    dbc.Button("Download Filtered CSV", id="download-csv-btn", color="success",
                        className="mt-4 w-100 btn-custom", ), dcc.Download(id="download-csv"), ], width=2, ), ],
                    align="center", )])], className="mb-4 shadow", ), # KPI Row
                dbc.Row([dbc.Col(html.Div(id="kpi-row"), width=12)], className="mb-4"), # Tabs for charts
                dbc.Card([dbc.CardBody([dcc.Tabs(id="tabs", value="tab-overview", className="mb-3", children=[
                    dcc.Tab(label="Overview", value="tab-overview", className="py-2 custom-tab", children=[dbc.Row(
                    ...
                    dcc.Tab(label="Trends", value="tab-trends", className="py-2 custom-tab", children=[dbc.Row(
                    ...
                    dcc.Tab(label="Comparisons", value="tab-compare", className="py-2 custom-tab", children=[dbc.Row(
                    ...
                    dcc.Tab(label="Export Chart", value="tab-export", className="py-2 custom-tab", children=[dbc.Row([
                        dbc.Col([dbc.Card([dbc.CardBody([dbc.Label("Select chart to export as PNG"),
                            dcc.Dropdown(id="export-fig-selector",
                                options=[{"label": "Leave Taken (Bar)", "value": "fig-leave-bar", },
                                    ...
                                    {"label": "Cumulative Attendance (Line)", "value": "fig-cumulative-attendance", },
                                    {"label": "Employee Heatmap", "value": "fig-heatmap-employee", },
                                    {"label": "Multi-Employee Attendance", "value": "fig-multi-employee-attendance", },
                                    {"label": "Top Leave Months", "value": "fig-top-leave-months", }, ],
                                value="fig-attendance-perc", clearable=False, className="mb-3", ),
                            dbc.Button("Download PNG", id="download-png-btn", color="primary",
                                className="w-100 btn-custom", ), dcc.Download(id="download-png"), ])])], width=4, ), dbc.Col(
                            [dbc.Alert([html.H6("Export Tip", className="alert-heading", ),
                                html.P("PNG export requires the 'kaleido' package.", className="mb-0", ),
                                html.Code("pip install kaleido", className="mt-2", ), ], color="info", )],
                            width=8, ), ])], ),
                    dcc.Tab(label="Consolidated Report", value="tab-consolidated", className="py-2 custom-tab", children=[
                        dbc.Row([dbc.Col([dbc.Card([dbc.CardBody([dbc.Row([dbc.Col([dbc.Label("Select Year:"),
                            dcc.Dropdown(id="consolidated-year-dropdown",
                                options=[{"label": y, "value": y, } for y in years], value=years[-1], clearable=False, ), ],
                            width=2, ), dbc.Col([dbc.Label("Select Month:"), dcc.Dropdown(id="consolidated-month-dropdown",
                            options=[{"label": m, "value": m, } for m in month_order], value=month_order[0],
                            clearable=False, ), ], width=2, ), dbc.Col([dbc.Label("Calculation Base:"),
                            dcc.Dropdown(id="consolidated-calculation-base-dropdown",
                                options=[{"label": "Total Working Days", "value": "Total Working Days", },
                                    {"label": "Total Billable Days", "value": "Total Billable Days", }, ],
                                value="Total Working Days", clearable=False, ), ], width=2, ), dbc.Col(
                            [dbc.Label("Attendance Threshold (%)"),
                                dcc.Slider(id="consolidated-threshold-slider", min=50, max=100, step=1, value=90,
                                    marks={x: str(x) for x in range(50, 101, 10, )},
                                    tooltip={"placement": "bottom", "always_visible": True, }, ), ], width=4, ), dbc.Col([
                            dbc.Button("Download CSV", id="download-consolidated-csv-btn", color="success",
                                className="mt-4 w-100 btn-custom", ), dcc.Download(id="download-consolidated-csv"), ],
                            width=2, ), ], align="center", )])], className="mb-3", )], width=12, )]), dbc.Row([dbc.Col([
                            dbc.Card([dbc.CardBody(
                                [html.Div(id="consolidated-table-container", className="consolidated-table")])])],
                            width=12, )]), ], ), ], )])], className="shadow", ),
                # Hidden stores to pass figures for PNG export
                dcc.Store(id="store-fig-leave-bar"), dcc.Store(id="store-fig-leave-vs-working-line"),
                dcc.Store(id="store-fig-attendance-perc"), dcc.Store(id="store-fig-attendance-trend-ma"),
                dcc.Store(id="store-fig-mom-change"), dcc.Store(id="store-fig-cumulative-attendance"),
                dcc.Store(id="store-fig-heatmap-employee"), dcc.Store(id="store-fig-multi-employee-attendance"),
        ...
        # -------------------------
        @dash_app.callback(Output("kpi-row", "children"), Output("fig-leave-bar", "figure"),
            Output("fig-leave-vs-working-line", "figure"), Output("fig-attendance-perc", "figure"),
            Output("fig-attendance-trend-ma", "figure"), Output("fig-mom-change", "figure"),
            Output("fig-cumulative-attendance", "figure"), Output("fig-heatmap-employee", "figure"),
            Output("fig-multi-employee-attendance", "figure"), Output("fig-top-leave-months", "figure"),
            # also store figs as JSON for PNG export
            Output("store-fig-leave-bar", "data"), Output("store-fig-leave-vs-working-line", "data"),
            Output("store-fig-attendance-perc", "data"), Output("store-fig-attendance-trend-ma", "data"),
            Output("store-fig-mom-change", "data"), Output("store-fig-cumulative-attendance", "data"),
            Output("store-fig-heatmap-employee", "data"), Output("store-fig-multi-employee-attendance", "data"),
            Output("store-fig-top-leave-months", "data"), Input("employee-dropdown", "value"),
            Input("year-dropdown", "value"), Input("calculation-base-dropdown", "value"),
        ...
        def update_all(selected_employee, selected_year, calculation_base, threshold):
            ...
            if (calculation_base == "Total Billable Days" and "Total Billable Days" in df_emp.columns):
                df_emp["Attendance %"] = 100 - (df_emp["Leave Taken Days"] / df_emp["Total Billable Days"] * 100)
                df_year["Attendance %"] = 100 - (df_year["Leave Taken Days"] / df_year["Total Billable Days"] * 100)
            else:
                # Default to Total Working Days
                df_emp["Attendance %"] = 100 - (df_emp["Leave Taken Days"] / df_emp["Total Working Days"] * 100)
                df_year["Attendance %"] = 100 - (df_year["Leave Taken Days"] / df_year["Total Working Days"] * 100)
            ...
            kpi_cards = dbc.Row([dbc.Col(kpi_card("Total Leave Days", total_leave, "danger"), width=3),
                dbc.Col(kpi_card("Average Attendance %", f"{avg_att}%", "primary"), width=3),
                dbc.Col(kpi_card("Lowest Attendance Month", min_att_month, "warning"), width=3), dbc.Col(
                    kpi_card("Alerts (<80%)", ", ".join(alerts) if alerts else "None",
                        "success" if not alerts else "danger", ), width=3, ), ], className="g-4", )
            ...
            fig_leave_bar = px.bar(df_emp, x="Month_Year", y="Leave Taken Days", text="Leave Taken Days",
                ...
                labels={"Month_Year": "Month", "Leave Taken Days": "Days"}, hover_data=["Dates of Leave", calculation_base],
            ...
            comparison_column = (calculation_base if calculation_base in df_emp.columns else "Total Working Days")

            fig_line = px.line(df_emp, x="Month_Year", y=["Leave Taken Days", comparison_column], markers=True,
            ...
            fig_att_perc = px.bar(df_emp, x="Month_Year", y="Attendance %", text=df_emp["Attendance %"].round(1),
            ...
            fig_cum = px.line(df_emp, x="Month_Year", y="Cum_Att", markers=True,
                ...
                labels={"Month_Year": "Month", "Cum_Att": "Cumulative %"}, color_discrete_sequence=["#17becf"], )
            ...
            if not df_emp.empty:
                ...
                fig_heat = px.imshow(heat_df, text_auto=True, labels={"x": "Month", "y": "Employee", "color": "Leave Days"},
                    title=f"Leave Heatmap — {selected_employee} ({selected_year})", color_continuous_scale="Reds", )
            ...
            if (calculation_base == "Total Billable Days" and "Total Billable Days" in df_year.columns):
                multi = df_year.groupby(["Full Name", "Month_Year"], as_index=False).agg(
                    {"Leave Taken Days": "sum", "Total Billable Days": "sum"})
                multi["Attendance %"] = 100 - (multi["Leave Taken Days"] / multi["Total Billable Days"] * 100)
            else:
                multi = df_year.groupby(["Full Name", "Month_Year"], as_index=False).agg(
            ...
            fig_top = px.bar(top5, x="Month_Year", y="Leave Taken Days", text="Leave Taken Days",
                ...
                labels={"Month_Year": "Month", "Leave Taken Days": "Days"}, color_discrete_sequence=["#e377c2"], )
            ...
            store_data = [fig_leave_bar.to_plotly_json(), fig_line.to_plotly_json(), fig_att_perc.to_plotly_json(),
                fig_trend.to_plotly_json(), fig_mom.to_plotly_json(), fig_cum.to_plotly_json(), fig_heat.to_plotly_json(),
                fig_multi.to_plotly_json(), fig_top.to_plotly_json(), ]

            return (
                kpi_cards, fig_leave_bar, fig_line, fig_att_perc, fig_trend, fig_mom, fig_cum, fig_heat, fig_multi, fig_top,
                *store_data,)

        # -------------------------
        # 4) CSV download for filtered employee/year
        # -------------------------
        @dash_app.callback(Output("download-csv", "data"), Input("download-csv-btn", "n_clicks"),
            State("employee-dropdown", "value"), State("year-dropdown", "value"),
            State("calculation-base-dropdown", "value"), prevent_initial_call=True, )
        def download_csv(n, employee, year, calculation_base):
            dff = df[(df["Full Name"] == employee) & (df["Year"] == year)].copy()

            # Calculate attendance percentage based on selected calculation base
            if (calculation_base == "Total Billable Days" and "Total Billable Days" in dff.columns):
                dff["Attendance %"] = 100 - (dff["Leave Taken Days"] / dff["Total Billable Days"] * 100)
            else:
                dff["Attendance %"] = 100 - (dff["Leave Taken Days"] / dff["Total Working Days"] * 100)
        ...
        # -------------------------
        @dash_app.callback(Output("download-png", "data"), Input("download-png-btn", "n_clicks"),
            State("export-fig-selector", "value"), State("store-fig-leave-bar", "data"),
            State("store-fig-leave-vs-working-line", "data"), State("store-fig-attendance-perc", "data"),
            State("store-fig-attendance-trend-ma", "data"), State("store-fig-mom-change", "data"),
            State("store-fig-cumulative-attendance", "data"), State("store-fig-heatmap-employee", "data"),
            State("store-fig-multi-employee-attendance", "data"), State("store-fig-top-leave-months", "data"),
            prevent_initial_call=True, )
        def download_png(n, fig_key, *stored_figs):
            key_map = {"fig-leave-bar": 0, "fig-leave-vs-working-line": 1, "fig-attendance-perc": 2,
                "fig-attendance-trend-ma": 3, "fig-mom-change": 4, "fig-cumulative-attendance": 5, "fig-heatmap-employee": 6,
                "fig-multi-employee-attendance": 7, "fig-top-leave-months": 8, }
        ...
        # -------------------------
        @dash_app.callback(Output("consolidated-table-container", "children"), Input("consolidated-year-dropdown", "value"),
            Input("consolidated-month-dropdown", "value"), Input("consolidated-calculation-base-dropdown", "value"),
        ...
        def update_consolidated_table(selected_year, selected_month, calculation_base, threshold):
            ...
            table_header = [html.Thead(html.Tr(
                [html.Th("Full Name"), html.Th("Leave Taken Days"), html.Th(calculation_base), html.Th("Attendance %"), ],
                className="table-primary table-header", ))]

            rows = []
            for _, row in dff.iterrows():
                attendance_color = ("text-danger" if row["Attendance %"] < threshold else "text-success")
                rows.append(html.Tr([html.Td(row["Full Name"]), html.Td(row["Leave Taken Days"]),
                    html.Td(row[calculation_base] if calculation_base in row else "N/A"),
                    html.Td(f"{row['Attendance %']:.1f}%", className=f"font-weight-bold {attendance_color}", ), ]))
        ...
        # -------------------------
        @dash_app.callback(Output("download-consolidated-csv", "data"), Input("download-consolidated-csv-btn", "n_clicks"),
            State("consolidated-year-dropdown", "value"), State("consolidated-month-dropdown", "value"),
            State("consolidated-calculation-base-dropdown", "value"), prevent_initial_call=True, )
        def download_consolidated_csv(n_clicks, year, month, calculation_base):
            ...
            if dff.empty:
                dff = pd.DataFrame(columns=["Full Name", "Leave Taken Days", calculation_base, "Attendance %"])
        ...
        dialog = QDialog()
        dialog.setWindowTitle("User Dashboard")
